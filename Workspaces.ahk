...
; working gist: https://gist.github.com/tdalon/60da20e1e2203ded15ab2c4b8bde33fe#file-workspace-ahk
...
capture(){
	...
	Loop,%list%{
		...
		if(title=""){ ; TD: if(max!=0||title="") - does not work e.g. does not display Outlook
			;MsgBox %title%
			continue	
		}
		; TD: if(title="program manager")
		if(title="program manager") || (title="Microsoft Edge") || (title="Setup") || (title="Microsoft Store") || (title="Settings") || (title="VirtuaWinMainClass") || (InStr(title,"Reminder(s)"))
			Continue
...
Gui(){
	...
	WinHide, % hwnd([1]) ; TD Hide after start
	return
	contract:
...
hwnd(win,hwnd=""){
	...
	if IsObject(win)
		return "ahk_id " window[win.1] ; TD add blank 
	if !hwnd
		return window[win]
	window[win]:=hwnd
}
